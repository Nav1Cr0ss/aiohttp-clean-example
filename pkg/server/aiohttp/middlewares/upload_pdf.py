...
async def upload_pdf(request: Request, handler: callable) -> Response:
    ...
    if request.content.is_eof():
        raise we.HTTPBadRequest(text="Missing file in request")

    if not filename:
        raise we.HTTPBadRequest(text="Missing filename in the request")
    ...
    if content_length > 104857600:
        raise we.HTTPRequestEntityTooLarge(
            max_size=104857600, actual_size=content_length
        )
