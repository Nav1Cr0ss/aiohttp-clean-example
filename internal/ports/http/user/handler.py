...
class UserHandler(UserHandlerIface):
    ...
    async def get_users(self, request: Request) -> Response:
        ...
        return Response(
            text=users.model_dump_json(), content_type="application/json", status=200
        )
    ...
    async def login_user(self, request: Request) -> Response:
        ...
        try:
            ...
            return Response(
                text=str(ue), content_type="application/json", status=ue.code
            )
    ...
    async def partial_update_user(self, request: Request) -> Response:
        ...
        if user.role != RoleEnum.admin:
            return Response(
                text="forbidden", content_type="application/json", status=403
            )
