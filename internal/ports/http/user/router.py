...
def get_routes(h: UserHandlerIface):
    routes = [
        ...
        Route("/users/{id}", PATCH, h.partial_update_user, middlewares=(construct_request_body,)),
        Route("/users/register", POST, h.create_user, no_auth=True, middlewares=(construct_request_body,)),
        Route("/users/login", POST, h.login_user, no_auth=True, middlewares=(construct_request_body,)),
    ]
    return routes
