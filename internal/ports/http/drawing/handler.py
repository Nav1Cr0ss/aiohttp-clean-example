...
from internal.ports.http.drawing.dto.drawing_history_filter_param import (
    DrawingHistoryFilterParam,
)
...
class DrawingHandler(DrawingHandlerIface):
    ...
    async def get_drawings(self, request: Request) -> Response:
        ...
        return Response(
            text=drawings.model_dump_json(), content_type="application/json", status=200
        )
    ...
    async def get_drawings_history(self, request: Request) -> Response:
        ...
        drawings_history: FileHistoryListSchema = await self.app.get_drawings_history(
            filter_params
        )
        return Response(
            text=drawings_history.model_dump_json(),
    ...
    async def get_drawing_history(self, request: Request) -> Response:
        ...
        try:
            drawing_history = await self.app.get_drawing_history(
                drawing_history_param.id
            )
        except ApiError as ae:
            return Response(text=str(ae), status=ae.code)

        return Response(
            text=drawing_history.model_dump_json(),
            content_type="application/json",
            status=200,
        )
    ...
    async def upload_drawing(self, request: Request) -> Response:
        ...
        file_id = await self.app.upload_drawing(
            file_stream, content_type, filename, user.id
        )

        return Response(
            text=json.dumps({"file_id": file_id}),
            content_type="application/json",
            status=200,
        )
    ...
    async def upload_drawings(self, request: Request) -> Response:
        ...
        async for file in await request.multipart():
            ...
            # Just example of validations, i would prefer to create middleware for such cases
            content_type = file.headers.get("Content-Type")
            if not content_type:
                return Response(text="Missing Content Type", status=400)

            file_name = file.filename
            if not file_name:
                return Response(text="Missing file name", status=400)

            file_id = await self.app.upload_drawing(
                file, content_type, file_name, user.id
            )
            file_ids.append(file_id)
        return Response(
            text=json.dumps({"file_ids": file_ids}),
            content_type="application/json",
            status=200,
        )
